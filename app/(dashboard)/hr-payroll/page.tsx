      ...
      <div className="premium-page p-4 sm:p-6 md:p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6">
          ...
          <div className="bg-card rounded-xl p-4 sm:p-5 border border-border">
            ...
            <p className="text-lg sm:text-2xl font-semibold text-foreground mt-1">{payrollRecords.length}</p>
          </div>
          <div className="bg-card rounded-xl p-4 sm:p-5 border border-border">
            <p className="text-xs sm:text-sm text-muted-foreground">Leave Requests</p>
        ...
        <Tabs defaultValue="payroll" className="space-y-4">
          <TabsList className="w-full sm:w-auto">
            <TabsTrigger value="payroll" className="flex-1 sm:flex-none">Payroll</TabsTrigger>
            <TabsTrigger value="leave" className="flex-1 sm:flex-none">Leave Management</TabsTrigger>
          </TabsList>
                  ...
                  <TableBody>
                    {paginatedPayrollRecords.map((record) => (
                      <TableRow key={record.id}>
                        ...
                        <TableCell className="hidden md:table-cell">৳{record.baseSalary.toLocaleString()}</TableCell>
                        <TableCell className="hidden md:table-cell text-green-600">+৳{record.bonus.toLocaleString()}</TableCell>
                        <TableCell className="hidden md:table-cell text-red-600">-৳{record.deductions.toLocaleString()}</TableCell>
              ...
              {payrollRecords.length > 0 && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 px-4 py-3 border-t border-border text-sm text-muted-foreground">
                  <span className="text-xs sm:text-sm">
                    Showing {(payrollPage - 1) * PAGE_SIZE + 1}–{Math.min(payrollPage * PAGE_SIZE, payrollRecords.length)} of {payrollRecords.length}
                  </span>
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" disabled={payrollPage === 1} onClick={() => setPayrollPage((p) => Math.max(1, p - 1))}>
                      Previous
                    </Button>
                    <span className="text-xs">Page {payrollPage} of {payrollTotalPages}</span>
                    <Button variant="outline" size="sm" disabled={payrollPage === payrollTotalPages} onClick={() => setPayrollPage((p) => Math.min(payrollTotalPages, p + 1))}>
                      Next
                    </Button>
                  </div>
                </div>
          ...
          <TabsContent value="leave">
            <div className="bg-card rounded-xl border border-border">
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden sm:table-cell"><span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-primary/60" />Days</span></TableHead>
                      <TableHead><span className="flex items-center gap-1.5"><Zap className="w-3.5 h-3.5 text-primary/60" />Status</span></TableHead>
                      <TableHead className="w-12"></TableHead>
                    </TableRow>
                  </TableHeader>
                    ...
                    {paginatedLeaveRecords.map((record) => (
                      <TableRow key={record.id}>
                        <TableCell className="font-medium">
                          <span className="truncate block max-w-[100px] sm:max-w-none">{record.employeeName}</span>
                        </TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 rounded-md text-xs font-medium whitespace-nowrap ${getLeaveTypeColor(record.type)}`}>
                            {record.type}
                          </span>
                        </TableCell>
                        <TableCell>
                          <div className="flex flex-col gap-0.5 text-sm">
                            ...
                            <span className="text-muted-foreground text-xs whitespace-nowrap">{record.endDate}</span>
                          </div>
                        </TableCell>
                        <TableCell className="hidden sm:table-cell">{record.days} day(s)</TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap ${getStatusColor(record.status)}`}>
                            {record.status}
                          </span>
              ...
              {leaveRecords.length > 0 && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 px-4 py-3 border-t border-border text-sm text-muted-foreground">
                  <span className="text-xs sm:text-sm">
                    Showing {(leavePage - 1) * PAGE_SIZE + 1}–{Math.min(leavePage * PAGE_SIZE, leaveRecords.length)} of {leaveRecords.length}
                  </span>
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" disabled={leavePage === 1} onClick={() => setLeavePage((p) => Math.max(1, p - 1))}>
                      Previous
                    </Button>
                    <span className="text-xs">Page {leavePage} of {leaveTotalPages}</span>
                    <Button variant="outline" size="sm" disabled={leavePage === leaveTotalPages} onClick={() => setLeavePage((p) => Math.min(leaveTotalPages, p + 1))}>
                      Next
                    </Button>
                  </div>
                </div>
