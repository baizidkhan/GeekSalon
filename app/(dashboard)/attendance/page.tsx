...
const attendanceRateData = MONTHS.map((month, mi) => ({
  month,
  onTime: Math.floor(seededRandom(mi * 3) * 30 + 50),
...
function generateDays(empIndex: number): DayStatus[] {
  return Array.from({ length: 31 }, (_, i) => {
    if (i >= 28) return "off"
    const r = seededRandom(empIndex * 31 + i)
